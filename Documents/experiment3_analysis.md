...
| :--- | :--- | :--- |
| | Y 轴 | 101.2 |
| **Anisotropic** | **X 轴 (Fiber)** | **232.2** |
| | Y 轴 (Transverse) | 100.0 |

### 3.2 结果讨论
- **方向响应性**：
    - 在各向异性配置下，**X 轴（硬轴）的刚度约为 Y 轴（软轴）的 2.32 倍**。这证明了 Proposed Method 能够根据输入的纤维方向参数产生非对称的力学响应。
    - 实验中观察到 Isotropic 情况下 X/Y 刚度比也接近 2.29，这主要归因于肝脏网格本身的几何不对称性以及边界固定条件的分布（网格在 X 方向具有更高的结构刚度）。
- **模型有效性**：
    - 启用各向异性后，X 方向的刚度维持在较高水平。这表明 Proposed Method 的各向异性算子能够在有限元框架中整合纤维增强贡献，维持预期的方向性差异。
    - 该特性对于实现**患者特异性（Patient-Specific）**仿真必要，因为它允许直接利用医学影像（如 DTI 数据）获取的纤维方向来驱动实时模拟。
