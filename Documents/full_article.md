...
实验结果表明，在 20k 四面体的肝脏模型上，本方法达到 **219 FPS**（单帧 4.6ms），相比全局隐式 FEM 的 5.5 FPS 提升了 **40 倍**。
...
- **体积守恒验证**：在实验 2 中，我们展示了在 $\nu = 0.49$ 的近不可压缩设置下，本方法的体积变化率 < 2.6%，而 XPBD 的体积误差约 7.7%。
...
1. **实验 1（大变形精度）**：在本数据集中，本方法的平均位移误差（相对于 VegaFEM 真值）约为 570%，而 XPBD Fast 的误差达 1390% 以上。
2. **实验 2（体积守恒）**：在 $\nu \ge 0.47$ 的设置下，本方法的体积变化率曲线平稳，峰值偏差 < 2.6%。
3. **实验 3（各向异性响应）**：在各向异性配置下，沿 X 轴和 Y 轴拖拽产生的力-位移曲线斜率比为 2.3，展现了明显的方向性差异。
4. **实验 4（多核加速）**：在 ~8k 四面体下达到 219 FPS，展示了极佳的实时性能。
...
四个验证实验表明：
- **精度提升**：相比 XPBD Fast 的巨大位移误差，本方法表现出更好的刚度维持能力。
- **稳定性增强**：体积变化率 < 2.6%（XPBD 约为 7.7%）。
- **物理准确**：各向异性力-位移比达到 2.3。
- **性能突破**：在 10 核 CPU 上达到 219 FPS（~8k Tets）。
