...
| :--- | :--- | :--- | :--- | :--- |
| 16 | 0.033 | 0.230 | 0.759 | 0.166 |
| 30 | 0.060 | 0.397 | 0.713 | 0.206 |
| 40 | 0.079 | 0.509 | 0.758 | 0.236 |
...
**结论**：在该数据集中，各方法均表现出一定的位移偏差，但 **Proposed Method** 的相对误差（平均约 **570%**）仍显著低于实时配置下的 **XPBD Fast**（平均约 **1393%**）。这表明在当前参数配置下，虽然绝对精度有待对齐，但本方法在维持结构刚度方面优于同类实时算法。
...
| **XPBD (Fast)** | ~6.4 | ~156.3 | 是 |
| **XPBD (Ref)** | ~50.3 | ~19.9 | **否** |
