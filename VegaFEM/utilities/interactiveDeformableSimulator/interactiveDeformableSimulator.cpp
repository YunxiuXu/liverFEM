...
// Experiment 1 parameters (config-tunable)
int exp1_settleSteps = 120;
int exp1_dragSteps = 240;
int exp1_holdSteps = 240;
double exp1_forceInfluenceRadius = 0.6; // fraction of bbox diagonal
double exp1_sweepAccel1 = 800.0;
double exp1_sweepAccel2 = 1500.0;
double exp1_sweepAccel3 = 2000.0;
int exp1_resetAfterFinish = 1;
int exp1_pbdIterationsFast = 30;
int exp1_pbdIterationsReference = 150;
double exp1_isotropicE = 1e6; // Young's modulus for isotropic material
double exp1_isotropicNu = 0.3; // Poisson's ratio for isotropic material
...
struct Experiment1State
{
  ...
  // State machine: 0=idle, 1=waiting, 2=apply_run_reset, 3=settle, 4=drag, 5=hold, 6=capture, 7=save_complete
  int state = 0;
  int stageStep = 0;
  
  // Nested loop control
  int accelIndex = 0; // outer loop: acceleration values
  int runTypeIndex = 0; // inner loop: 0=fast, 1=reference
  double accelTarget = 0.0;
  int currentIterations = 0; // current PBD iterations
  std::vector<double> sweepAccels;
  std::vector<int> runTypes; // 0=fast, 1=reference
  
  // Target vertex and force application
  int targetVertex = -1; // 0-indexed
  double pullRadius = 0.0;
  Vec3d pullCenter;
  std::vector<double> massDiag; // length 3*n, for converting accel->force
  double currentLoadScale = 0.0; // 0.0 to 1.0, linear ramp during drag
  
  // Data collection
  std::string basePrefix;
  std::ofstream positionsCsv;
  std::ofstream summaryCsv;
  std::vector<std::vector<double>> referencePositions; // reference solution positions (all vertices)
  std::vector<std::vector<double>> fastPositions; // fast solution positions (all vertices)
  
  // Material parameters to restore
  std::vector<double> prevE;
  std::vector<double> prevNu;
  std::vector<double> prevDensity;
  
  // restore previous state
  int prevStaticSolver = 0;
  int prevAddGravity = 0;
  int prevMaxIterations = 1;
  double prevEpsilon = 1e-6;
};
...
static void Experiment1_SetStatus(const std::string & s)
{
...
static double BBoxDiagonal(const VolumetricMesh * mesh)
{
  Vec3d bmin, bmax;
  GetBoundingBox(mesh, bmin, bmax);
...
static Vec3d PickPullCenterAtMaxY(const VolumetricMesh * mesh, const std::vector<int> & fixedVertices0Indexed)
{
  const int nLocal = mesh->getNumVertices();
  std::vector<char> isFixed(nLocal, 0);
  for (int v : fixedVertices0Indexed)
    if (v >= 0 && v < nLocal)
      isFixed[v] = 1;
  ...
  for (int i = 0; i < nLocal; i++)
  {
    if (isFixed[i])
      continue;
    if (best < 0 || mesh->getVertex(i)[1] > mesh->getVertex(best)[1])
      best = i;
  }
  if (best < 0)
    best = 0;
  return mesh->getVertex(best);
}

static std::vector<int> BuildVerticesWithinRadius(const VolumetricMesh * mesh, const std::vector<int> & fixedVertices0Indexed, const Vec3d & center, double radius, int minCount)
{
  const int nLocal = mesh->getNumVertices();
  ...
  if ((int)vertices.size() < minCount)
  {
    std::vector<std::pair<double,int>> scored;
    scored.reserve(nLocal);
    for (int i = 0; i < nLocal; i++)
    {
      if (isFixed[i])
        continue;
      ...
      const double dy = p[1] - center[1];
      const double dz = p[2] - center[2];
      scored.push_back({dx*dx + dy*dy + dz*dz, i});
    }
    std::sort(scored.begin(), scored.end(), [](const auto & a, const auto & b){ return a.first < b.first; });
    vertices.clear();
    for (int k = 0; k < (int)scored.size() && (int)vertices.size() < minCount; k++)
      vertices.push_back(scored[k].second);
  }
...
static void WritePositionsCSV(const std::string & filename, const VolumetricMesh * mesh, const double * uDeformation)
{
  ...
  for (int i = 0; i < nLocal; i++)
  {
    const Vec3d & p0 = mesh->getVertex(i);
    const double x = p0[0] + uDeformation[3*i+0];
    const double y = p0[1] + uDeformation[3*i+1];
    const double z = p0[2] + uDeformation[3*i+2];
    out << (i+1) << "," << x << "," << y << "," << z << "\n";
  }
}

static std::vector<double> ExtractDiagonal(const SparseMatrix * M)
{
  std::vector<double> diag(M->GetNumRows(), 0.0);
  for (int row = 0; row < M->GetNumRows(); row++)
  {
    const int len = M->GetRowLength(row);
    for (int j = 0; j < len; j++)
    {
      const int col = M->GetColumnIndex(row, j);
      if (col == row)
      {
        diag[row] = M->GetEntry(row, j);
        break;
      }
    }
  }
...
// Select target vertex: prefer Z in front 80% region with max X, else max X overall
static int SelectTargetVertex(const VolumetricMesh * mesh, const std::vector<int> & fixedVertices0Indexed)
{
  const int nLocal = mesh->getNumVertices();
  std::vector<char> isFixed(nLocal, 0);
  for (int v : fixedVertices0Indexed)
  ...
  Vec3d minBound(DBL_MAX, DBL_MAX, DBL_MAX);
  Vec3d maxBound(-DBL_MAX, -DBL_MAX, -DBL_MAX);
  for (int i = 0; i < nLocal; i++)
  {
    if (isFixed[i])
      continue;
    ...
    for (int d = 0; d < 3; d++)
    {
      if (p[d] < minBound[d]) minBound[d] = p[d];
      if (p[d] > maxBound[d]) maxBound[d] = p[d];
    }
  }
  
  // Z depth and threshold for front 80%
  double depth = maxBound[2] - minBound[2];
  double zThreshold = minBound[2] + depth * 0.8; // front 80% region
  
  // First pass: find max X in front 80% region
  int best = -1;
  ...
  // If not found, use max X overall
  if (best < 0)
  {
    for (int i = 0; i < nLocal; i++)
    {
...
void idleFunction(void)
{
  ...
  const bool runSimulation = (!lockScene) && (singleStepMode <= 1) && (experiment1.active || (!pauseSimulation));
  if (runSimulation)
  {
    // determine force in case user is pulling on a vertex
    if ((!experiment1.active) && g_iLeftMouseButton) 
    {
      if (pulledVertex != -1)
      {
    ...
    // Experiment 1: quasi-static pull (apply acceleration on a radius-based region).
    // Experiment 1: Apply force with spherical falloff
    if (experiment1.active && experiment1.targetVertex >= 0 && experiment1.targetVertex < n)
    {
      if (experiment1.state == 4 || experiment1.state == 5) // drag or hold stage
      {
        const Vec3d & targetPos0 = volumetricMesh->getVertex(experiment1.targetVertex);
        const double * uq = integratorBase->Getq();
        Vec3d targetPos(
          targetPos0[0] + uq[3*experiment1.targetVertex+0],
          targetPos0[1] + uq[3*experiment1.targetVertex+1],
          targetPos0[2] + uq[3*experiment1.targetVertex+2]
        );
        
        const double accel = experiment1.accelTarget * experiment1.currentLoadScale;
        const Vec3d forceDir(1.0, 0.0, 0.0); // X direction
        
        // Apply force to all vertices within radius
        for (int v = 0; v < n; v++)
        {
          ...
          Vec3d dist = p - targetPos;
          double distLen = len(dist);
          
          if (distLen <= experiment1.pullRadius)
          {
            double falloff = 1.0 - (distLen / experiment1.pullRadius);
            double forceScale = falloff;
            
            // Target vertex gets 1.5x enhancement
            if (v == experiment1.targetVertex)
              forceScale *= 1.5;
            
            const double my = (3*v+0 >= 0 && 3*v+0 < 3*n) ? experiment1.massDiag[3*v+0] : 0.0;
    ...
    for(int i=0; i<substepsPerTimeStep; i++)
    {
      ...
      if (!experiment1.active) { printf("."); fflush(nullptr); }
    ...
    if (!experiment1.active) { printf("F"); fflush(nullptr); }
    graphicFrame++;
...
void initConfigurations()
{
  ...
  // Experiment 1 (optional) - match your config schema
  configFile.addOptionOptional("exp1_settleSteps", &exp1_settleSteps, exp1_settleSteps);
  configFile.addOptionOptional("exp1_dragSteps", &exp1_dragSteps, exp1_dragSteps);
  configFile.addOptionOptional("exp1_holdSteps", &exp1_holdSteps, exp1_holdSteps);
  configFile.addOptionOptional("exp1_forceInfluenceRadius", &exp1_forceInfluenceRadius, exp1_forceInfluenceRadius);
  configFile.addOptionOptional("exp1_sweepAccel1", &exp1_sweepAccel1, exp1_sweepAccel1);
  configFile.addOptionOptional("exp1_sweepAccel2", &exp1_sweepAccel2, exp1_sweepAccel2);
  configFile.addOptionOptional("exp1_sweepAccel3", &exp1_sweepAccel3, exp1_sweepAccel3);
  configFile.addOptionOptional("exp1_resetAfterFinish", &exp1_resetAfterFinish, exp1_resetAfterFinish);
  configFile.addOptionOptional("exp1_pbdIterationsFast", &exp1_pbdIterationsFast, exp1_pbdIterationsFast);
  configFile.addOptionOptional("exp1_pbdIterationsReference", &exp1_pbdIterationsReference, exp1_pbdIterationsReference);
...
static void Experiment1_SaveMaterialParams()
{
  if (volumetricMesh == nullptr)
    return;
  
  experiment1.prevE.clear();
  experiment1.prevNu.clear();
  experiment1.prevDensity.clear();
  
  for (int i = 0; i < volumetricMesh->getNumMaterials(); i++)
  {
    VolumetricMesh::Material * mat = volumetricMesh->getMaterial(i);
    VolumetricMesh::ENuMaterial * eNuMat = downcastENuMaterial(mat);
    if (eNuMat != nullptr)
    {
      experiment1.prevE.push_back(eNuMat->getE());
      experiment1.prevNu.push_back(eNuMat->getNu());
      experiment1.prevDensity.push_back(mat->getDensity());
    }
  }
}

// Restore material parameters
static void Experiment1_RestoreMaterialParams()
{
  if (volumetricMesh == nullptr || experiment1.prevE.empty())
    return;
  
  // Restore first material (assuming single material mesh)
  if (volumetricMesh->getNumMaterials() > 0 && experiment1.prevE.size() > 0)
  {
    volumetricMesh->setSingleMaterial(experiment1.prevE[0], experiment1.prevNu[0], experiment1.prevDensity[0]);
  }
}

// Compute RMSE between two position arrays (2D: [[x0,y0,z0], [x1,y1,z1], ...])
static double ComputeRMSE(const std::vector<std::vector<double>> & pos1, const std::vector<std::vector<double>> & pos2)
{
  ...
  {
    if (pos1[i].size() != pos2[i].size() || pos1[i].size() != 3)
      continue;
    for (size_t j = 0; j < 3; j++)
    {
      double diff = pos1[i][j] - pos2[i][j];
      sumSq += diff * diff;
      totalElements++;
    }
  }
  return (totalElements > 0) ? sqrt(sumSq / totalElements) : 0.0;
}

// Compute max error (2D: [[x0,y0,z0], [x1,y1,z1], ...])
static double ComputeMaxError(const std::vector<std::vector<double>> & pos1, const std::vector<std::vector<double>> & pos2)
{
  if (pos1.size() != pos2.size() || pos1.size() == 0)
    return 0.0;
  ...
  for (size_t i = 0; i < pos1.size(); i++)
  {
    if (pos1[i].size() != pos2[i].size() || pos1[i].size() != 3)
      continue;
    Vec3d v1(pos1[i][0], pos1[i][1], pos1[i][2]);
    Vec3d v2(pos2[i][0], pos2[i][1], pos2[i][2]);
    double err = len(v1 - v2);
    if (err > maxErr)
      maxErr = err;
  }
  return maxErr;
...
// Get all vertex positions as vector
static std::vector<double> GetAllPositions(const VolumetricMesh * mesh, const double * uDeformation)
{
  std::vector<double> positions;
  const int nLocal = mesh->getNumVertices();
  positions.reserve(3 * nLocal);
  for (int i = 0; i < nLocal; i++)
  {
    const Vec3d & p0 = mesh->getVertex(i);
    positions.push_back(p0[0] + uDeformation[3*i+0]);
    positions.push_back(p0[1] + uDeformation[3*i+1]);
    positions.push_back(p0[2] + uDeformation[3*i+2]);
  }
  return positions;
}

static void Experiment1_StopAndRestore()
{
  if (!experiment1.active)
    return;

  experiment1.active = false;
  experiment1.state = 0;
  experiment1.stageStep = 0;
  experiment1.accelIndex = 0;
  experiment1.runTypeIndex = 0;
  experiment1.accelTarget = 0;
  experiment1.currentIterations = 0;
  experiment1.sweepAccels.clear();
  experiment1.runTypes.clear();
  experiment1.targetVertex = -1;
  experiment1.currentLoadScale = 0.0;
  experiment1.referencePositions.clear();
  experiment1.fastPositions.clear();
  
  if (experiment1.positionsCsv.is_open())
    experiment1.positionsCsv.close();
  if (experiment1.summaryCsv.is_open())
    experiment1.summaryCsv.close();
  experiment1.massDiag.clear();

  // Restore material parameters
  Experiment1_RestoreMaterialParams();

  addGravity = experiment1.prevAddGravity;
  staticSolver = experiment1.prevStaticSolver;
  if (implicitNewmarkSparse != nullptr)
  {
    implicitNewmarkSparse->UseStaticSolver(staticSolver);
    implicitNewmarkSparse->SetMaxIterations(experiment1.prevMaxIterations);
    implicitNewmarkSparse->SetEpsilon(experiment1.prevEpsilon);
  }
}

static void Experiment1_WriteIndexListBou(const std::string & filename, const std::vector<int> & vertices0Indexed)
{
  std::ofstream out(filename.c_str());
  for (int i = 0; i < (int)vertices0Indexed.size(); i++)
  {
    out << (vertices0Indexed[i] + 1);
    out << ",";
  }
  out << "\n";
}
...
static void Experiment1_OnTimestepCompleted()
{
  ...
  if (experiment1.targetVertex < 0 || experiment1.targetVertex >= n)
    return;

  const double * uq = integratorBase->Getq();
  
  // State machine implementation
  if (experiment1.state == 1) // waiting - transition to apply_run_reset
  {
    ...
    experiment1.state = 3; // settle
    char msg[512];
    const char * runName = (experiment1.runTypes[experiment1.runTypeIndex] == 0) ? "fast" : "reference";
    std::snprintf(msg, sizeof(msg), "Experiment 1: settle | accel=%.0f %s iter=%d", 
                  experiment1.accelTarget, runName, experiment1.currentIterations);
    Experiment1_SetStatus(msg);
    return;
  }

  if (experiment1.state == 3) // settle
  {
    experiment1.stageStep++;
    if (experiment1.stageStep >= exp1_settleSteps)
    {
      experiment1.state = 4; // drag
      experiment1.stageStep = 0;
      experiment1.currentLoadScale = 0.0;
      Experiment1_SetStatus("Experiment 1: drag");
    }
    return;
  }

  if (experiment1.state == 4) // drag - linear ramp load
  {
    experiment1.stageStep++;
    experiment1.currentLoadScale = (double)experiment1.stageStep / exp1_dragSteps;
    if (experiment1.currentLoadScale > 1.0)
      experiment1.currentLoadScale = 1.0;
    
    if (experiment1.stageStep >= exp1_dragSteps)
    {
      experiment1.state = 5; // hold
      experiment1.stageStep = 0;
      experiment1.currentLoadScale = 1.0;
      Experiment1_SetStatus("Experiment 1: hold");
    }
    return;
  }

  if (experiment1.state == 5) // hold
  {
    experiment1.stageStep++;
    if (experiment1.stageStep >= exp1_holdSteps)
    {
      experiment1.state = 6; // capture
      experiment1.stageStep = 0;
      Experiment1_SetStatus("Experiment 1: capture snapshot");
    }
    return;
  }

  if (experiment1.state == 6) // capture snapshot
  {
    // Get all positions
    std::vector<double> positions = GetAllPositions(volumetricMesh, uq);
    
    // Compute target displacement
    const Vec3d & p0 = volumetricMesh->getVertex(experiment1.targetVertex);
    Vec3d pCurrent(
      p0[0] + uq[3*experiment1.targetVertex+0],
      p0[1] + uq[3*experiment1.targetVertex+1],
      p0[2] + uq[3*experiment1.targetVertex+2]
    );
    Vec3d displacement = pCurrent - p0;
    double targetDisplacement = len(displacement);
    
    // Save to positions CSV
    int runIndex = experiment1.accelIndex * 2 + experiment1.runTypeIndex;
    const char * runName = (experiment1.runTypes[experiment1.runTypeIndex] == 0) ? "fast" : "reference";
    experiment1.positionsCsv << runIndex << "," << runName << "," << experiment1.accelTarget 
                             << "," << experiment1.currentIterations << "," << targetDisplacement;
    
    // Write initial and current positions
    for (int i = 0; i < n; i++)
    {
      const Vec3d & pInit = volumetricMesh->getVertex(i);
      experiment1.positionsCsv << "," << pInit[0] << "," << pInit[1] << "," << pInit[2];
      experiment1.positionsCsv << "," << positions[3*i+0] << "," << positions[3*i+1] << "," << positions[3*i+2];
    }
    experiment1.positionsCsv << "\n";
    
    // Store positions for error calculation (convert 1D to 2D: [x0,y0,z0,x1,y1,z1,...] -> [[x0,y0,z0], [x1,y1,z1], ...])
    const int nLocal = volumetricMesh->getNumVertices();
    if (experiment1.runTypes[experiment1.runTypeIndex] == 0)
    {
      experiment1.fastPositions.clear();
      experiment1.fastPositions.reserve(nLocal);
      for (int i = 0; i < nLocal; i++)
      {
        experiment1.fastPositions.push_back({positions[3*i+0], positions[3*i+1], positions[3*i+2]});
      }
    }
    else
    {
      experiment1.referencePositions.clear();
      experiment1.referencePositions.reserve(nLocal);
      for (int i = 0; i < nLocal; i++)
      {
        experiment1.referencePositions.push_back({positions[3*i+0], positions[3*i+1], positions[3*i+2]});
      }
    }
    
    experiment1.state = 7; // save_complete
    return;
  }
  ...
  if (experiment1.state == 7) // save_complete
  {
    ...
    // Advance to next run
    experiment1.runTypeIndex++;
    if (experiment1.runTypeIndex >= (int)experiment1.runTypes.size())
    {
      experiment1.runTypeIndex = 0;
      experiment1.accelIndex++;
      
      if (experiment1.accelIndex >= (int)experiment1.sweepAccels.size())
      {
        // All done
        if (exp1_resetAfterFinish)
          stopDeformations_buttonCallBack(0);
        Experiment1_StopAndRestore();
        Experiment1_SetStatus("Experiment 1: done (files written)");
        return;
      }
    }
...
void experiment1_buttonCallBack(int code)
{
  ...
  {
    Experiment1_SetStatus("Experiment 1: unavailable (no volumetric mesh loaded)");
    return;
  }
  ...
  {
    Experiment1_StopAndRestore();
    Experiment1_SetStatus("Experiment 1: idle");
    return;
  }

  // Save state we will override
  experiment1.prevStaticSolver = staticSolver;
  experiment1.prevAddGravity = addGravity;
  experiment1.prevMaxIterations = maxIterations;
  experiment1.prevEpsilon = epsilon;
  Experiment1_SaveMaterialParams();

  // Reset to rest
  stopDeformations_buttonCallBack(0);

  // Select target vertex (Z in front 80%, max X)
  std::vector<int> fixedVertices0Indexed;
  fixedVertices0Indexed.reserve(numFixedVertices);
  for (int i = 0; i < numFixedVertices; i++)
    fixedVertices0Indexed.push_back(fixedVertices[i]);
  
  experiment1.targetVertex = SelectTargetVertex(volumetricMesh, fixedVertices0Indexed);
  if (experiment1.targetVertex < 0)
  {
    Experiment1_SetStatus("Experiment 1: failed to find target vertex");
    return;
  }

  // Compute pull radius
  const double diag = BBoxDiagonal(volumetricMesh);
  experiment1.pullRadius = std::max(1e-9, exp1_forceInfluenceRadius * diag);
  experiment1.pullCenter = volumetricMesh->getVertex(experiment1.targetVertex);
  ...
  experiment1.positionsCsv << "runIndex,runName,accel,iterations,targetDisplacement";
  const int nLocal = volumetricMesh->getNumVertices();
  for (int i = 0; i < nLocal; i++)
  {
    const Vec3d & p0 = volumetricMesh->getVertex(i);
    experiment1.positionsCsv << ",v" << (i+1) << "_x0,v" << (i+1) << "_y0,v" << (i+1) << "_z0";
    experiment1.positionsCsv << ",v" << (i+1) << "_x,v" << (i+1) << "_y,v" << (i+1) << "_z";
  }
  experiment1.positionsCsv << "\n";

  experiment1.summaryCsv.open((experiment1.basePrefix + "_sweep_summary.csv").c_str());
  experiment1.summaryCsv << "accel,runType,iterations,RMSE,maxError,targetDisplacementFast,targetDisplacementRef,relativeError,rmseNormalized,maxErrorNormalized\n";

  // Write metadata
  std::ofstream metadata((experiment1.basePrefix + "_metadata.txt").c_str());
  metadata << "Experiment 1 Metadata\n";
  metadata << "====================\n";
  metadata << "Target Vertex: " << (experiment1.targetVertex + 1) << "\n";
  metadata << "Target Position: " << volumetricMesh->getVertex(experiment1.targetVertex) << "\n";
  metadata << "Pull Radius: " << experiment1.pullRadius << "\n";
  metadata << "Time Step: " << timeStep << "\n";
  metadata << "Settle Steps: " << exp1_settleSteps << "\n";
  metadata << "Drag Steps: " << exp1_dragSteps << "\n";
  metadata << "Hold Steps: " << exp1_holdSteps << "\n";
  metadata << "Fast Iterations: " << exp1_pbdIterationsFast << "\n";
  metadata << "Reference Iterations: " << exp1_pbdIterationsReference << "\n";
  metadata << "Isotropic E: " << exp1_isotropicE << "\n";
  metadata << "Isotropic Nu: " << exp1_isotropicNu << "\n";
  metadata.close();

  // Initialize experiment state
  experiment1.active = true;
  experiment1.state = 1; // waiting
  experiment1.stageStep = 0;
  experiment1.accelIndex = 0;
  experiment1.runTypeIndex = 0;
  experiment1.sweepAccels = { exp1_sweepAccel1, exp1_sweepAccel2, exp1_sweepAccel3 };
  experiment1.runTypes = { 0, 1 }; // 0=fast, 1=reference
  experiment1.currentLoadScale = 0.0;
  experiment1.referencePositions.clear();
  experiment1.fastPositions.clear();

  // Precompute mass diagonal
  experiment1.massDiag = ExtractDiagonal(massMatrix);
...
void initGLUI()
{
  ...
  GLUI_Panel * experiments_panel = glui->add_panel("Experiments", GLUI_PANEL_EMBOSSED);
  experiments_panel->set_alignment(GLUI_ALIGN_LEFT);
