...
\uppercase{Yunxiu Xu}\authorrefmark{1}, \IEEEmembership{Graduate Student Member, IEEE},
...
From the computational perspective, soft tissues such as the liver exhibit pronounced large deformation and near-incompressible behavior during pressing and traction. Real-time simulation therefore faces a dual constraint of accuracy and efficiency. Continuum FEM offers strong physical interpretability and is well suited to representing local stiffness changes and material-parameter differences, but traditional global solvers are expensive. To address this issue, prior research has explored corotational FEM, cutting FEM, and grouped solving strategies to preserve physical credibility under real-time constraints \cite{bui2019corotational,wang2025group}. At the same time, Projective Dynamics, PBD, and XPBD offer advantages in interactive speed, numerical stability, and implementation convenience \cite{bouaziz2023projective,liu2016towards,bender2014survey,muller2007position,macklin2016xpbd}. However, when the goal is to represent lesion stiffness rather than merely visually plausible motion, material interpretability and mechanical consistency remain central.

From the interaction perspective, high-DOF force-feedback devices can provide richer kinesthetic and contact information, but they usually come with engineering costs such as structural complexity, high price, and restricted workspace \cite{agboh2016six}. Review studies likewise indicate that medical training systems do not necessarily benefit from blindly pursuing the highest number of degrees of freedom or the highest possible fidelity. What matters more is whether the critical perceptual cues are emphasized for a specific task \cite{overtoom2019haptic,rangarajan2020systematic}. Recent work also stresses task-oriented force-profile design rather than reconstructing every haptic dimension by default \cite{bjelland2025haptic}. This provides theoretical support for the 1-DOF interface used here: if lesion search depends primarily on local differences in normal resistance, then low-complexity hardware may still play a meaningful role in preclinical validation.

Overall, existing studies provide foundations in real-time soft-tissue modeling, liver-related training tasks, and medical haptic-system design, but relatively few works integrate a physically meaningful continuum model, consumer-grade interactive large-deformation updating, and a lightweight haptic loop into a single task-oriented system framework.

Based on this motivation, this paper proposes and implements a task-oriented haptic training prototype to evaluate the system connectivity and engineering feasibility of integrating continuum physics computation into a lightweight haptic loop. The prototype uses liver lesion localization as a concrete test scenario, adopts GB-cFEM as the core physical engine, combines it with Stereo IR-170 bimanual tracking and a haptic interface, simulates hidden lesions via local modulus enhancement, and maps contact-normal reaction force to haptic output. The evaluation in this paper is a formative expert evaluation whose goal is to identify system feasibility and engineering boundaries. The main contributions are as follows:
\begin{enumerate}
\item A task-oriented GB-cFEM physical-haptic closed-loop system prototype is proposed and implemented. Contact-normal force is extracted from a continuum physics engine and translated into force output, demonstrating the system connectivity of this computational architecture for palpation interaction and enabling real-time deployment on compact consumer-grade hardware.
\item A haptic mapping and exploration mechanism for lesion localization is designed. By combining local stiffness encoding with a fingertip-worn haptic interface, the system supports real-time extraction and feedback of hidden-lesion mechanical features and demonstrates the application potential of local hardness differences as palpation cues.
...
Along this line of reducing haptic dimensionality or emphasizing specific cues, reality-based force profiles have been used to simplify kinesthetic rendering in surgical simulation \cite{bjelland2025haptic}. These studies suggest that low-DOF haptic output is not merely a simplified substitute, but can also be understood as a task-oriented design choice for highlighting perceptual cues such as normal resistance and stiffness contrast.

\subsection{Position of This Work}
In summary, existing studies have established important foundations in real-time soft-tissue modeling, liver-related training tasks, and medical haptic rendering. However, there is still room to explore system-level integration of a physics engine and a lightweight haptic loop under consumer-grade hardware, together with system-level diagnosis. Based on this gap, this paper develops a task-oriented haptic training prototype using liver lesion localization as the test scenario and focuses on validating the system integration and usability of a physics-computation-driven interaction architecture. The evaluation in this paper is a formative expert evaluation whose goal is to identify system feasibility and engineering boundaries rather than to demonstrate training efficacy.
...
In this paper, ``XPBD Fast'' refers to the typical real-time configuration used in the corresponding script (\texttt{subSteps = 5}, \texttt{maxIterations = 1}), whereas ``XPBD Ref'' refers to a higher-accuracy reference configuration under the same mesh and the same fixation strategy but with XPBD substeps increased substantially to 50 in order to observe its error and cost near convergence.

Both XPBD configurations are reported to avoid the bias of comparing only one setting. If only the low-substep configuration were reported, XPBD would be placed at an obvious accuracy disadvantage because of its emphasis on real time. If only the high-substep configuration were reported, its interactive cost would be obscured. Reporting both ``Fast'' and ``Ref'' makes the typical tradeoff of XPBD between real-time performance and stiffness/deformation accuracy easier to interpret fairly and clarifies the relative advantage of the method used in this paper.
...
This evaluation was a formative expert evaluation. Its goal was to identify system feasibility and engineering boundaries rather than to prove training effectiveness. The evaluation setup is shown in Fig.~\ref{fig:eval}. The expert first watched an approximately 5-minute system demonstration and then completed, in sequence, free exploration, a blind lesion-localization observation, and a usability evaluation task.
...
The formative evaluation in this paper records three kinds of indicators: behavioral indicators (changes in lesion-search strategy), subjective evaluation (verbal feedback during free exploration and semi-structured interview), and rating-scale scores (evaluation of soft-tissue information expression and system performance). The assessment emphasizes qualitative observation and system-mechanism analysis in order to provide diagnostic and design guidance for the next generation of the system.
...
The second issue concerned local perceptual differences caused by simplified anatomical constraints. The expert observed that when the liver was pushed from the side, the model exhibited global translation similar to a floating object rather than rotation or local deformation constrained by anterior ligament traction. In fact, the boundary conditions in the physical model were simplified through an equivalent representation. Instead of adopting computationally expensive one-dimensional ligament models, fixed points were directly assigned to the Porta Hepatis (vascular root anchor), the Bare Area (a broad superior adhesion region), and the Groove for the Inferior Vena Cava (major venous support). Specifically, fixing the Porta Hepatis is equivalent to locking the structural pivot at the bottom of the liver; fixing the whole surface of the Bare Area computationally covers the tensile limiting effect that would otherwise be imposed by its marginal ligaments; and fixing points along the IVC groove is equivalent to providing a vertical load-bearing support on the posterior side. This surface-based macroscopic fixation strategy already captures the main traction-limiting roles of the supporting ligaments at the mechanical level and is used to support stable interaction under large deformation. The expert's feedback suggests that, although this physical equivalent simplification is mechanically reasonable at a macroscopic level, the absence of anterior traction constraints can still produce a mismatch between local physical feedback and actual clinical logic during specific lateral pushing maneuvers.
...
These comments define the current use scenario of the system as a targeted training prototype focused on expressing lesion-related haptic characteristics and clarify the mechanical and boundary conditions that still need to be added before approaching clinical-grade surgical simulation.
...
According to the Likert-scale scores in Table~\ref{tab:likert}, the system received positive ratings in ``architectural feasibility'' and ``soft-tissue mechanical expression'' (4/5), while the score for ``force-feedback realism'' was only moderate (3/5). This result is not intended to prove the absolute superiority of the system. Rather, it objectively confirms that the data flow of the system has been established and clarifies the engineering difficulties that the next-generation system must overcome in hardware frequency response and feedback dimensionality.

\section{Discussion}
\subsection{From Algorithm Validation to System-Prototype Evaluation}
This point needs to be stated clearly. The prior GB-cFEM work addressed real-time large-deformation physical solving, and the previous haptic-device work addressed lightweight fingertip output. The new work of this paper lies in the task-oriented integration of a medical palpation scenario, the normal-force-mapping closed loop, deployment on consumer-grade hardware, and formative expert diagnosis.
...
First, the absence of material nonlinearity blurs the perception of lesion depth and stiffness. The current model deforms too easily under compression and lacks the progressive compressive resistance and capsule-related limitation of real tissue during deep pressing. In the localization task, this linear response makes it difficult for the operator to probe deep hard nodules through changes in pressing resistance and therefore weakens palpation-based discrimination of deep lesions.

Second, simplified anatomical constraints such as anterior traction affect recognition of marginal lesions. Although the current anchoring strategy guarantees macroscopic stability, the organ can still undergo global translation rather than local stretch during lateral pushing. When a lesion is located near the liver edge, the operator may feel reaction force dominated by organ displacement rather than by the local hardness abnormality of the lesion itself, which interferes with search judgment near the organ boundary.

Third, the normal-force limitation of 1-DOF feedback reduces the ability to perceive lesion boundaries. Because tangential force and lateral squeezing cues are absent, the operator cannot perceive the resistance-gradient change or the spatial sense of envelopment while the finger slides across the edge of a hard region. This increases the difficulty of outlining a lesion in three dimensions and forces the operator to rely on dense vertical pressing to infer the boundary.

These mechanisms, which are difficult to expose through numerical tests alone, reveal how engineering simplifications can change the behavior pattern of lesion localization. The construction of the present system prototype therefore not only verifies the connectivity of the data flow, but also provides concrete targets for future system evolution through diagnostic analysis.

\subsection{Application Focus and Future Evolution of the System}
For the test scenario of liver lesion localization, the current system expresses lesion characteristics through normal-force-based stiffness feedback. At the same time, what is validated here is not an isolated implementation for one specific liver model, but a transferable way of organizing the system. As long as the target task also depends on local stiffness differences, contact-reaction cues, and real-time updatable soft-tissue deformation, the architecture of ``GB-cFEM physical kernel + local property encoding + haptic mapping'' may be extended to other soft-tissue palpation or lesion-search scenarios. Of course, such extension is not unconditional and still requires organ-specific remodeling of geometry, boundary constraints, material parameters, and interaction paradigm. To further improve the immersion of surgical simulation as a whole, future evolution may proceed in the following directions: first, introducing more complex material models with capsule effects and nonlinear compressive behavior; second, adding more realistic ligament and abdominal support constraints to strengthen organ anchoring; third, extending the interaction paradigm smoothly from bare-hand palpation to instrument-dominant workflows; and fourth, while preserving the lightweight advantage of the 1-DOF solution, further improving tangential compensation and continuity in feedback control.

\subsection{Future Research Directions}
Future work may further examine task validity and training effectiveness under a larger sample, but the scope of the present paper is limited to system-prototype design and technical feasibility validation. The next stage of engineering iteration will prioritize the following directions: first, enriching ligament and abdominal-boundary modeling to improve the macroscopic physical support of the organ; second, exploring surgical-instrument proxy interaction to correct the current interaction paradigm; and finally, optimizing the synchronization mechanism between the underlying physics engine and the haptic serial loop to improve the continuity of force rendering.
